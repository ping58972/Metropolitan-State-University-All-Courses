...
public class StringBox {
    ...
    public static void main(String[] args) {
        ...
        // String strNum = "21462675756";
        String strNum = "98676555533";
        String result = "";
        Deque<Character> q = new LinkedList<>();
        for (char c : strNum.toCharArray()) {
            if (q.isEmpty())
                q.add(c);
            else if ( ((int) q.peekLast() % 2 == 0) && (c % 2 == 0)) {
                q.add('*');
                q.add(c);
            } else if ( ((int) q.peekLast() % 2 != 0) && (c % 2 != 0)) {
                q.add('-');
    ...
    // * with the maximum values possible (6 if possible), and when we hit the "lower bound"
    // (meaning the
