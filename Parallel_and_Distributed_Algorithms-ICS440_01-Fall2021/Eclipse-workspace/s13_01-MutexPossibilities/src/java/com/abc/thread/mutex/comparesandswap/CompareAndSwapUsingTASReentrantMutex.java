...
public class CompareAndSwapUsingTASReentrantMutex implements Mutex {
    private static final long THREAD_ID_NO_ONE = 0;

    private final CompareAndSwapUsingTestAndSet cas;
    private int lockDepth;

    public CompareAndSwapUsingTASReentrantMutex() {
        cas = new CompareAndSwapUsingTestAndSet(THREAD_ID_NO_ONE);
        lockDepth = 0;
    }
    ...
    public void lock() {
        ...
        if (cas.compareAndSwap(threadId, threadId)) {
        ...
        while ( !cas.compareAndSwap(THREAD_ID_NO_ONE, threadId)) spinCount++; // spin
    ...
    public void unlock() {
        long threadId = Thread.currentThread().getId();
        if (cas.compareAndSwap(threadId, threadId)) {
            // the calling thread *IS* the current lock holder
            lockDepth--;
            if (lockDepth == 0) {
                cas.compareAndSwap(threadId, THREAD_ID_NO_ONE);
